...
const SIGNAL = {
  ...
  // The 91 word-list pages. "list" is fired on load, not on a click: the
  // question these answer is not "did they click" but the prior one, "has a
  // human being ever opened one of these at all" — 61 recorded visits in 24h
  // and the only filter behind that number is "did not say bot in the
  // user-agent", which Amazonbot walks straight through.
  onList: /^\/px\/list\.gif$/,
  fromList: /^\/px\/listclick\.gif$/,
};
...
// 8. A word-list page. These are generated by scripts/word-list-pages.mjs from
// one template, so one page proves all 91 — but the generator is run by hand
// and the beacon lives in its template, so a regenerate that drops it would be
// invisible. This is the check that it did not.
{
  const ctx = await browser.newContext({ viewport: { width: 390, height: 664 } });
  const page = await ctx.newPage();
  await page.route("https://static.cloudflareinsights.com/**", (route) => route.abort());
  ...
  const has = (re) => [...paths].some((p) => re.test(p));
  check(has(SIGNAL.onList), "opening a word-list page fires the arrival beacon");
  check(!has(SIGNAL.fromList), "and does not claim the button was pressed");
  await page.click("a.btn");
  await page.waitForTimeout(1200);
  console.log(`  word-list page: ${Object.entries(SIGNAL).filter(([, re]) => has(re)).map(([k]) => k).join(", ")}`);
  check(has(SIGNAL.fromList), "the word-list button fires its beacon and survives the navigation");
  check(has(SIGNAL.ranTheApp), "and lands on the generator");
  await ctx.close();
}
