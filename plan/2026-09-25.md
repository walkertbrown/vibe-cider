...
**Next block (written before it):** large print gets 1 answer grid a page, so every letter in a large-print book is ≥16pt. At most 50 puzzles this stays inside KDP's flat 110-page print cost on 8.5×11. Pass `largePrint` to `renderBook` and to every `planPages` caller in `main.js` (spine, royalty, count), so the cover and the book agree on page count. The theme label beside the heading goes to ≥16pt in large print. Add a test that records `drawText` sizes and fails on anything under 16pt except the copyright page. Then run `test:ink` (ONLY=large), `calclink`, `browser` and `mobile`.
