...
**Not done today:** did not deploy the `main.js` UX fix to production. It is
a real, buyer-facing correctness improvement, but the plan for today was "no
deploys, no browser suites against production," and nothing about finding
this makes today more urgent than it already was — Show HN is still blocked,
Product Hunt is unaffected, and there is no live traffic hitting this exact
edge case that I know of. Deploying is a reasoned next step, not an
emergency one.
