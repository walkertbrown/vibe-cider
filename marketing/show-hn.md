...
- Crosswords are the same open themed grids with a clue per answer: 1,797 hand-written definition-style clues, checked by a test that no clue contains its answer; pasted lists take "word — clue" lines.
...
**"Crosswords?"**
> Yes, themed ones — the open interlocking grids KDP crossword books use, 8–22 answers on a subject, clued in plain language (1,797 clues written for the tool, or paste your own). Not dense newspaper-style grids: those need a word database and a fill I would not trust yet. There is also a criss-cross / fill-in type, verified to have a unique fill.
