...
>
> On the puzzles themselves. Mazes are spanning trees, so every one has exactly one route from start to finish and no unreachable corners. Every sudoku is dug out one symmetric pair of clues at a time, and a clue is only removed if the grid still solves exactly one way — a puzzle with two answers would make the solutions page at the back of your own book wrong. Word search draws on 86 themes or a list you paste. All three can be graded — easy at the front working up to expert at the back, the way published puzzle books are usually built, with the level printed on each puzzle.
>
...
- Likely questions, worth having answers ready for:
  ...
  - *What about large print?* There's a Large print checkbox — 8.5×11, fourteen words a grid, letters around 22pt against about 14 at 6×9 (it varies with the word list — say "around 22", never a flat number). Note it moves you to large trim, which costs more per page to print; the royalty line in the tool shows that as it happens.
  ...
  - *Isn't there a free tool that does this?* Yes, and for some people it's the better answer — PuzzleForge is free, no account, no watermark, six types, and bulk-generates up to 50 puzzles with answer keys. The difference is puzzles versus a book: its own FAQ says "Save PDF" opens a print window and tells you to choose "Save as PDF" as the printer, so you get puzzle pages — no title or copyright page, no page numbers, no inside margin by page count, no even page count, and no cover. The cover is the wall, because the spine width comes from the page count of the book you just made. Side-by-side: /compare
    - **The site is `the-puzzle-forge.org`, not `puzzleforge.app`** — different product, almost the same name. I checked the wrong one on 2026-09-14 and nearly weakened this answer over it. Re-verified on the right domain the same day: the FAQ quote and all of it still stands.
